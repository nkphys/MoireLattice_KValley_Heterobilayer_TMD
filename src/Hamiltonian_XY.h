...
void Hamiltonian::Update_Bloch_States_Using_Projection(Mat_1_Complex_doub & Psi_state_,int space_slices,double rx_min,double d_rx,double ry_min,double d_ry){
...
//	ry_offset=(0.5)*Parameters_.a_moire;
    rx_min=-8*Parameters_.a_moire + Wnr_center_x;
    ry_min=-8*Parameters_.a_moire; + Wnr_center_y;
    rx_max=8*Parameters_.a_moire + Wnr_center_x;
    ry_max=8*Parameters_.a_moire + Wnr_center_y;
    int r_ind;
    int space_slices=200;
    d_rx=(rx_max-rx_min)/(space_slices);
    d_ry=(ry_max-ry_min)/(space_slices);
	...
	assert((L1_eff*Parameters_.a_moire)<(rx_max-rx_min));
	assert((L2_eff*Parameters_.a_moire)<(ry_max-ry_min));
   ...
   //------------------------------------------
	...
	complex<double> phase_2=conj(Ham_(0, eigen_no-1))*(1.0/abs(Ham_(0,eigen_no-1)));
			...
			Psi_state_[r_ind] += Ham_(comp,eigen_no)*exp(iota_complex*(kx_local*(rx_min + rx_ind*d_rx) +  ky_local*(ry_min + ry_ind*d_ry) ));
                        ...
                        if( (abs(rx_min + rx_ind*d_rx - 0.0*Parameters_.a_moire)<=0.0000001) && 
		             (abs(ry_min + ry_ind*d_ry - 0.0*Parameters_.a_moire)<=0.0000001) ){
	...
	complex<double> checknorm2=0.0;
                        for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
                            for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
                                r_ind=rx_ind + (space_slices)*ry_ind;
                                checknorm2 +=   d_rx*d_ry*Psi_state_[r_ind]*conj(Psi_state_[r_ind]);
                            }
                        }
		for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
                            for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
    ...
    for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
        for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
            r_ind=rx_ind + (space_slices)*ry_ind;
            checknorm += d_rx*d_ry*Wnr_state_[r_ind]*conj(Wnr_state_[r_ind]);
        }
    }
    ...
    for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
     ...
     double rx_mean_temp, ry_mean_temp, r2_mean_temp;
     rx_mean_temp=0.0; ry_mean_temp=0.0;r2_mean_temp=0.0;
      for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
        for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
            r_ind=rx_ind + (space_slices)*ry_ind;
            rx_mean_temp += d_rx*d_ry*abs(Wnr_state_[r_ind])*abs(Wnr_state_[r_ind])*(rx_min + rx_ind*d_rx);
            ry_mean_temp += d_rx*d_ry*abs(Wnr_state_[r_ind])*abs(Wnr_state_[r_ind])*(ry_min + ry_ind*d_ry);
	    r2_mean_temp += d_rx*d_ry*abs(Wnr_state_[r_ind])*abs(Wnr_state_[r_ind])*
			((ry_min + ry_ind*d_ry)*(ry_min + ry_ind*d_ry) +
			(rx_min + rx_ind*d_rx)*(rx_min + rx_ind*d_rx)	);
        }
    }

    cout<<"rx_mean (using Wr) = "<<rx_mean_temp/Parameters_.a_moire<<endl;
    cout<<"ry_mean (using Wr) = "<<ry_mean_temp/Parameters_.a_moire<<endl;
    cout<<"r2_mean (using Wr) = "<<r2_mean_temp/(Parameters_.a_moire*Parameters_.a_moire)<<endl;	

    checknorm=0.0;
    string file_Wnr_out="Wannier_functions_band" + to_string(band) +".txt";
    ofstream FileWNROut(file_Wnr_out.c_str());
    FileWNROut<<"#index rx ry W_r  ....."<<endl;
    for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
        for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
            r_ind=rx_ind + (space_slices)*ry_ind;
            FileWNROut<<r_ind<<"  "<<(rx_min + rx_ind*d_rx)/(Parameters_.a_moire)<<"   "<<(ry_min + ry_ind*d_ry)/(Parameters_.a_moire)<<"   "<<Wnr_state_[r_ind].real()<<"   "<<Wnr_state_[r_ind].imag() <<endl;
            checknorm += d_rx*d_ry*Wnr_state_[r_ind]*conj(Wnr_state_[r_ind]);
        }
        FileWNROut<<endl;
    }
	...
	for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
                for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
                   r_ind=rx_ind + (space_slices)*ry_ind;
                   U_Bloch_mat[n1+(n2*L1p2)][r_ind]=exp(-1.0*iota_complex*( kx_*(rx_min + rx_ind*d_rx)  + ky_*(ry_min + ry_ind*d_ry)  ) )*Psi_state_[r_ind];
        }
        }
	}
	}
...
string file_BF_out="Bloch_U_functions_band" + to_string(band) +".txt";
    ...
    for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
        for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
            r_ind=rx_ind + (space_slices)*ry_ind;
            FileBFOut<<r_ind<<"  "<<(rx_min + rx_ind*d_rx)/(Parameters_.a_moire)<<"   "<<(ry_min + ry_ind*d_ry)/(Parameters_.a_moire)<<"   "<<U_Bloch_mat[3+((3)*L1p2)][r_ind].real()<<"   "<<U_Bloch_mat[3+((3)*L1p2)][r_ind].imag() <<endl;
...
for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
	...
	M_Mat_old[n_ind_][b_neigh]+= d_rx*d_ry*conj(U_Bloch_mat[n_ind][r_ind])*U_Bloch_mat[n_neigh_ind][r_ind];
	}
	}	
	}
...
for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
      for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
                   r_ind=rx_ind + (space_slices)*ry_ind;
                   Psi_state_[r_ind]=U_Bloch_mat[n1+n2*L1p2][r_ind]*exp(1.0*iota_complex*( kx_*(rx_min + rx_ind*d_rx) + ky_*(ry_min + ry_ind*d_ry) ) );
}
}
...
//Update_Bloch_States_Using_Projection(Psi_state_, space_slices, rx_min, d_rx, ry_min, d_ry);
    ...
    FileLWNROut<<"#index rx ry W_r  ....."<<endl;
    for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
        for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
            r_ind=rx_ind + (space_slices)*ry_ind;
            FileLWNROut<<r_ind<<"  "<<(rx_min + rx_ind*d_rx)/(Parameters_.a_moire)<<"   "<<(ry_min + ry_ind*d_ry)/(Parameters_.a_moire)<<"   "<<Wnr_state_[r_ind].real()<<"   "<<Wnr_state_[r_ind].imag() <<endl;
            checknorm += d_rx*d_ry*Wnr_state_[r_ind]*conj(Wnr_state_[r_ind]);
        }
        FileLWNROut<<endl;
...
rx_mean_temp=0.0; ry_mean_temp=0.0;r2_mean_temp=0.0;
    ...
    cout<<"r2_mean (using MLWR) = "<<r2_mean_temp/(Parameters_.a_moire*Parameters_.a_moire)<<endl;
...
//( kx_*((1.0/(2.0*sqrt(3.0)))*Parameters_.a_moire) + ky_*( (0.5)*Parameters_.a_moire )
      ...
      Wnr_state_[r_ind]*conj(Wnr_state_[r_ind]))
                            *exp(iota_complex*(qx_*((rx_min + rx_ind*d_rx - Wnr_center_x )) + qy_*((ry_min + ry_ind*d_ry  - Wnr_center_y ))));
                }
            }
...
//( kx_*((1.0/(2.0*sqrt(3.0)))*Parameters_.a_moire) + ky_*( (0.5)*Parameters_.a_moire )
      ...
      for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
      r_ind=rx_ind + (space_slices)*ry_ind;
      Mmq_SphC[q_ind][theta_ind] += d_rx*d_ry*(
      Wnr_state_[r_ind]*conj(Wnr_state_[r_ind]))
                            *exp(iota_complex*(-1.0*qx_*((rx_min + rx_ind*d_rx - Wnr_center_x )) - 1.0*qy_*((ry_min + ry_ind*d_ry  - Wnr_center_y ))));
                }
            }
...
double radius_ = (rx_max - Wnr_center_x)*0.5;
double center_x= Wnr_center_x;  
double center_y= Wnr_center_y;
...
for(int r1_=0;r1_<L1_eff;r1_++){
	...
	int R_i = r1_ +  r2_*L1_eff;



//( kx_*((1.0/(2.0*sqrt(3.0)))*Parameters_.a_moire) + ky_*( (0.5)*Parameters_.a_moire)
      MqR_SphC[q_ind][theta_ind][R_i]=0.0;
      for(int rx_ind=0;rx_ind<space_slices;rx_ind++){
      for(int ry_ind=0;ry_ind<space_slices;ry_ind++){
	...
	//if( ( rx_min + rx_ind*d_rx + dis_x)>(rx_min+d_rx*0.001) &&
	 //   ( rx_min + rx_ind*d_rx + dis_x)<(rx_max-d_rx*0.001) &&
	 //   ( ry_min + ry_ind*d_ry + dis_y)>(ry_min+d_ry*0.001) &&
         //   ( ry_min + ry_ind*d_ry + dis_y)<(ry_max-d_ry*0.001) )



	if( 
	( ( (((rx_min + rx_ind*d_rx - Wnr_center_x)*(rx_min + rx_ind*d_rx - Wnr_center_x))  
		+
	       ((ry_min + ry_ind*d_ry - Wnr_center_y)*(ry_min + ry_ind*d_ry - Wnr_center_y)))
	)<(radius_*radius_))
	
	&&

	(rx_min + rx_ind*d_rx + dis_x)>(rx_min+d_rx*0.001) &&
           ( rx_min + rx_ind*d_rx + dis_x)<(rx_max-d_rx*0.001) &&
            ( ry_min + ry_ind*d_ry + dis_y)>(ry_min+d_ry*0.001) &&
           ( ry_min + ry_ind*d_ry + dis_y)<(ry_max-d_ry*0.001)
	...
	int rx_ind_r = int((rx_ind*d_rx + dis_x + d_rx*0.5)/(d_rx));
        int ry_ind_r = int((ry_ind*d_ry + dis_y + d_ry*0.5)/(d_ry));


	//cout<<rx_ind_l<<"  "<<ry_ind_l<<"   "<<rx_ind_r<<"  "<<ry_ind_r<<"  "<<dis_x<<"  "<<dis_y<<endl;
//	cout<<(rx_ind_r-rx_ind_l)*d_rx<<"  "<< (ry_ind_r-ry_ind_l)*d_ry<<"  "<<dis_x<<"  "<<dis_y<<"  "<<d_rx<<"  "<<d_ry<<endl;	
...
//	int ry_ind_r = ry_ind;	
     ...
     int r_ind_=rx_ind + (space_slices)*ry_ind;
     int r_ind_r=rx_ind_r + (space_slices)*ry_ind_r;
      MqR_SphC[q_ind][theta_ind][center_neigh] += d_rx*d_ry*(
      Wnr_state_[r_ind_r]*conj(Wnr_state_[r_ind_]))
	//*cos((qx_*((rx_min + rx_ind*d_rx - Wnr_center_x )) + qy_*((ry_min + ry_ind*d_ry  - Wnr_center_y ))));   
*exp(iota_complex*(qx_*((rx_min + rx_ind*d_rx - Wnr_center_x )) + qy_*((ry_min + ry_ind*d_ry  - Wnr_center_y ))));
...
for(int r1_=0;r1_<L1_eff;r1_++){
	...
	val_temp  = (1.0/(4.0*PI*PI))*(V_*(conj(MqR_SphC[q_ind][theta_ind][center_temp]))*(MqR_SphC[q_ind][theta_ind][center_neigh]));

	AqSphCFILE<<q_val<<"  "<<theta_val<<"  "<<val_temp.real()<<"  "<<val_temp.imag() <<endl;
                            //*exp(-1.0*iota_complex*( qx_*(dis_x*0.5) +  qy_*(dis_y*0.5) ))<<endl; 
                }
		AqSphCFILE<<endl;
            }
        }
